...
def run_bash_cmd(cmd, echo=False, interaction: dict[str, str] = {}, return_lines=True, return_code=False, cr_as_newline=False):
    if options["VERBOSE"]:
        echo = True
    if echo:
        print("CMD:", cmd)
...
@app.command()
def repl():
    if options["CIRCUITPYTHON"]:
        repl_circuitpython()
    else:
        repl_micropython()


def shell_micropython():
